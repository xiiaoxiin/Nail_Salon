...
$sqlcheckcart = "SELECT * FROM tbl_products WHERE prid = '$prid' "; //check product in cart
        ...
        if ($prqty == 0) { //product is out of stock
           ...
           echo $sqlcheckcart = "SELECT * FROM tbl_carts WHERE prid = '$prid' AND email = '$email'"; //check if the product is already in cart
            ...
            if ($resultcart->num_rows == 0) {//product is not in the cart proceed with insert new
                ...
                echo $sqlupdatecart = "UPDATE tbl_carts SET cartqty = qty +1 WHERE prid = '$prid' AND email = '$email'";
