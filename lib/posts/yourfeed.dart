...
class YourFeed extends StatefulWidget {
  ...
  const YourFeed({Key key, this.user}) : super(key: key);
...
class _YourFeedState extends State<YourFeed> {
                        ...
                        Flexible(
                            flex: 9,
                             child: ListView.builder(
                                itemCount: _userlistfeeds.length,
                                      ...
                                      child: SingleChildScrollView(
                                        child: Column(
                                          children: [
                                                      ...
                                                      decoration: new BoxDecoration(
                                                          ...
                                                          image: new DecorationImage(
                                                              ...
                                                              image: new NetworkImage(
                                                                  "https://lowtancqx.com/s269957/nailsalon/images/profileimages/default.png")))),
                                                ),
                                                Expanded(
                                                    flex: 6,
                                                    child: Column(
                                            ...
                                            GestureDetector(
                                              onTap: () => _showFeed(index),
                                              child: Container(
                                                padding: EdgeInsets.all(5),
                                                alignment: Alignment.centerLeft,
                                                child: Text(checkFeedLen(
                                                    _userlistfeeds[index]
                                                        ['feeddesc'])),
                                              ),
                                            ),
                                            SizedBox(height: 10),
                                                  ...
                                                  children: [
                                                    TextButton(
                                                      ...
                                                      child: Row(
                                                        children: <Widget>[
                                                          Text(
                                                            _userlistfeeds[index]['numcomments'].toString(),
                                                            style: TextStyle(
                                                              color: Theme.of(
                                                                      context)
                                                                  .accentColor,
  ...
  String checkFeedLen(String feed) {
    if (feed.length < 35) {
      return feed;
    } else {
      return feed.substring(0, 35) + " ...read more";
    }
  }
  ...
  void _loadFeeds() {
    print(_pageno);
    http.post(
        Uri.parse( "https://lowtancqx.com/s269957/nailsalon/php/load_feed.php"),
        body: {
          "email": widget.user.email,
          "pageno": _pageno.toString(),
        }).then((response) {
      if (response.body == "nodata") {
        _titlecenter = "Sorry no feed available";
        return;
      } else {
        var jsondata = json.decode(response.body);
        _userlistfeeds = jsondata["feeds"];
        pagenum = _userlistfeeds[0]['numpage'];
        setState(() {});
        print(_userlistfeeds);
      }
    });
  }

  void _deleteDialog(int index) {
    print(_userlistfeeds[index]['feedid']);
    showDialog(
        context: context,
  ...
  Future<void> _deleteFeed(int index) async {
    ...
    http.post(
        Uri.parse(
             "https://lowtancqx.com/s269957/nailsalon/php/delete_feed.php"),
        body: {
          "feedid": _userlistfeeds[index]['feedid'],
        }).then((response) {
  ...
  void _editDialog(int index) {
    ...
    editdescCtrl.text = _userlistfeeds[index]['feeddesc'];
    showDialog(
      context: context,
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      String feeddesc = editdescCtrl.text.toString();
                      _updateFeed(index, feeddesc);
                    },
                    child: Text("Update"),
                  ),
                ),
  ...
  _updateFeed(int index, String newfeed) async {
    pr = ProgressDialog(context);
    pr.style(
    ...
    http.post(
        Uri.parse(
             "https://lowtancqx.com/s269957/nailsalon/php/update_feed.php"),
        body: {
          "feedid": _userlistfeeds[index]['feedid'],
          "newfeed": newfeed
        }).then((response) {
      ...
      builder: (context) => new AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Container(
            ...
            child: Column(
              children: [
                Expanded(flex: 1, child: Text("Feed")),
                Expanded(
                    flex: 7,
                    child: SingleChildScrollView(
                        child: Text(_userlistfeeds[index]['feeddesc']))),
                Expanded(
                    flex: 2,
                    child: Container(
                      width: screenWidth,
  ...
  void sendComment(String comment, int index, newSetState) {
    print(comment);
    print(_userlistfeeds[index]['user_email']);
    print(widget.user.email);
    print(_userlistfeeds[index]['feedid']);

    http.post(
        Uri.parse( "https://lowtancqx.com/s269957/nailsalon/php/insert_comment.php"),
        body: {
          "feed_id": _userlistfeeds[index]['feedid'],
          "feed_user": _userlistfeeds[index]['user_email'],
          "feed_reply": widget.user.email,
  ...
  Future<List> loadComment(int index) async => http.post(
          Uri.parse(
               "https://lowtancqx.com/s269957/nailsalon/php/load_comment.php"),
          body: {
            "feedid": _userlistfeeds[index]['feedid'],
          }).then((response) {
        List listComments;
